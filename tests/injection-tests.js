...
describe('Injection', function() {
	...
	it('should inject property', function() {
		...
		var container = new Container()
			.registerType(Foo, null, null, [ new Inject.Property('bar', 'Bar') ])
			.registerType(Bar);
	...
	it('should inject method with args', function() {
		...
		var container = new Container()
			.registerType(Foo, null, null, [ new Inject.Method('method', [ 'bat', 'baz' ]) ]);
	...
	it('should inject method', function() {
		...
		var container = new Container()
			.registerType(Foo, null, null, [ new Inject.Method('method') ])
			.registerType(Bar)
			.registerType(Baz);
	...
	it('should throw if method does not exist', function() {
		...
		(function() {
			new Container()
				.registerType(Foo, null, null, [ new Inject.Method('asdf') ])
				.resolve('Foo');
